...
export async function listUsers(request: Request, response: Response) {
  try {
    ...
    // Map response user representation
...
export async function updateUser(request: Request, response: Response) {
  ...
  const { role, isActive, isPending, weeklyTargetHours, name, email, notificationEmail, phoneNumber, location, bio, skills, avatarUrl } = request.body;
  ...
  try {
    ...
    const updatedUser = await prisma.user.update({
      ...
      data: {
        ...
        weeklyTargetHours: weeklyTargetHours !== undefined ? Number(weeklyTargetHours) : undefined,
    ...
    if (isNowApproved) {
      // Create database notification for approved user
      await createNotification({
        recipientId: updatedUser.id,
        title: "Account Activated",
        message: "Your Project Tracking Platform workspace profile has been approved and activated! Welcome aboard.",
      ...
      logActivity(
        ...
        null,
        null,
        `Approved registration request for "${updatedUser.name}" as ${updatedUser.role}`
      );
    } else {
    ...
    response.status(200).json({
      ...
      user: {
        ...
        weeklyTargetHours: updatedUser.weeklyTargetHours,
...
export async function changePassword(request: Request, response: Response) {
  ...
  try {
    if (!request.user || (request.user.userId !== id && request.user.role !== "ADMIN")) {
